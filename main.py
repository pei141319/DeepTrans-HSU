...
def run_cae_analysis():
    dataset_name = 'apex'  # 可根据需要修改
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    
    # 1. 运行 CAE 模型 (use_transformer=False)
    print(f"\n🚀 开始执行 CAE (Pure-CNN) 专项分析 - 数据集: {dataset_name}")
    cae_trainer = Trans_mod.Train_test(
    ...
    sad, rmse = cae_trainer.run(smry=False)
    
    # 2. 结果归档：将本次运行的图片重命名，防止下次被覆盖
    # 这样你可以在同一个文件夹里看到：cae_abundance.png
    target_dir = cae_trainer.save_dir
    
    if os.path.exists(os.path.join(target_dir, "abundance.png")):
        shutil.copy(os.path.join(target_dir, "abundance.png"), 
                    os.path.join(target_dir, "CAE_Abundance_Map.png"))
                    
    if os.path.exists(os.path.join(target_dir, "end_members.png")):
        shutil.copy(os.path.join(target_dir, "end_members.png"), 
                    os.path.join(target_dir, "CAE_Endmember_Spectra.png"))

    print("\n" + "="*50)
    print(f"📊 CAE 分析完成！")
    print(f"🔹 最终指标: SAD={sad:.4f}, RMSE={rmse:.4f}")
    print(f"📂 所有分析文件已保存在: {os.path.abspath(target_dir)}")
    print(f"   - 查看端元图: CAE_Endmember_Spectra.png")
    print(f"   - 查看丰度图: CAE_Abundance_Map.png")
    print("="*50)
